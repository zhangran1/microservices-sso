...
public class ZenikaGroupLDAPStorageMapper extends AbstractLDAPStorageMapper implements CommonLDAPGroupMapper {

    private static final Logger logger = Logger.getLogger(ZenikaGroupLDAPStorageMapper.class);

    private final GroupMapperConfig config;
    private final ZenikaGroupLDAPStorageMapperFactory factory;

    // Flag to avoid syncing multiple times per transaction
    private boolean syncFromLDAPPerformedInThisTransaction = false;

    public ZenikaGroupLDAPStorageMapper(ComponentModel mapperModel, LDAPStorageProvider ldapProvider, ZenikaGroupLDAPStorageMapperFactory factory) {
    ...
    public LDAPObject createLDAPGroup(String groupName, Map<String, Set<String>> additionalAttributes) {
        ...
        LDAPObject ldapGroup = ZenikaLDAPUtils.createLDAPGroup(ldapProvider, groupName, config.getGroupNameLdapAttribute(), config.getGroupObjectClasses(ldapProvider),
    ...
    public LDAPObject updateLDAPGroup(LDAPObject ldapObject) {
        LDAPObject ldapGroup = ZenikaLDAPUtils.updateLDAPGroup(ldapProvider, ldapObject);
    ...
    public SynchronizationResult syncDataFromFederationProviderToKeycloak(RealmModel realm) {
        logger.infof("------- syncDataFromFederationProviderToKeycloak");
    ...
    protected List<LDAPObject> getAllLDAPGroups(boolean includeMemberAttribute) {
        logger.infof("------------getAllLDAPGroups");
        try (LDAPQuery ldapGroupQuery = createGroupQuery(includeMemberAttribute)) {
            logger.infof("------------ldapGroupQuery =%s", ldapGroupQuery.toString());
            logger.infof("------------includeMemberAttribute =%s", includeMemberAttribute);
            return ZenikaLDAPUtils.loadAllLDAPObjects(ldapGroupQuery, ldapProvider);
        }
    }
    ...
    private void processKeycloakGroupMembershipsSyncToLDAP(GroupModel kcGroup, Map<String, LDAPObject> ldapGroupsMap) {
        ...
        for (GroupModel kcSubgroup : kcSubgroups) {
            ...
            if (!toRemoveSubgroupsDNs.remove(ldapSubgroup.getDn())) {
                ...
                ZenikaLDAPUtils.addMember(ldapProvider, MembershipType.DN, config.getMembershipLdapAttribute(), membershipUserLdapAttrName, ldapGroup, ldapSubgroup);
            }
        }
        ...
        for (LDAPDn toRemoveDN : toRemoveSubgroupsDNs) {
            ...
            ZenikaLDAPUtils.deleteMember(ldapProvider, MembershipType.DN, config.getMembershipLdapAttribute(), membershipUserLdapAttrName, ldapGroup, fakeGroup);
        }
    ...
    public void addGroupMappingInLDAP(RealmModel realm, GroupModel kcGroup, LDAPObject ldapUser) {
        ...
        if (ldapGroup == null) {
            ...
            if (config.isPreserveGroupsInheritance()) {
                ...
                if (highestGroupToSync.getParent() != groupsPathGroup) {
                    ...
                    ZenikaLDAPUtils.addMember(ldapProvider, MembershipType.DN, config.getMembershipLdapAttribute(), getMembershipUserLdapAttribute(), ldapParentGroup, ldapGroup);
                }
            } else {
        ...
        ZenikaLDAPUtils.addMember(ldapProvider, config.getMembershipTypeLdapAttribute(), config.getMembershipLdapAttribute(), membershipUserLdapAttrName, ldapGroup, ldapUser);
    }

    public void deleteGroupMappingInLDAP(LDAPObject ldapUser, LDAPObject ldapGroup) {
        String membershipUserLdapAttrName = getMembershipUserLdapAttribute();
        ZenikaLDAPUtils.deleteMember(ldapProvider, config.getMembershipTypeLdapAttribute(), config.getMembershipLdapAttribute(), membershipUserLdapAttrName, ldapGroup, ldapUser);
    }
    ...
    public void onImportUserFromLDAP(LDAPObject ldapUser, UserModel user, RealmModel realm, boolean isCreate) {
        ...
        logger.infof("----------------------ldapGroup isCreate=%s", isCreate);
        logger.infof("----------------------LDAPGroupMapperMode mode=%s", mode);
        ...
        if (mode == LDAPGroupMapperMode.IMPORT && isCreate) {
            ...
            for (LDAPObject ldapGroup : ldapGroups) {
                logger.infof("----------------------ldapGroup uuid = %s, dn=%s", ldapGroup.getUuid(), ldapGroup.getDn() );
    ...
    public class LDAPGroupMappingsUserDelegate extends UserModelDelegate {
        ...
        public void leaveGroup(GroupModel group) {
            try (LDAPQuery ldapQuery = createGroupQuery(true)) {
                ...
                String membershipUserLdapAttrName = getMembershipUserLdapAttribute();
                String membershipUserAttr = ZenikaLDAPUtils.getMemberValueOfChildObject(ldapUser, config.getMembershipTypeLdapAttribute(), membershipUserLdapAttrName);
