...
export class PostUpdateComponent implements OnInit {
  ...
  apponesSharedCollection: IAppone[] = [];
  ...
  trackApponeById(index: number, item: IAppone): number {
    return item.id!;
  }
  ...
  protected updateForm(post: IPost): void {
    this.editForm.patchValue({
      ...
      applicationone: post.applicationone,
      tags: post.tags,
    });

    this.apponesSharedCollection = this.apponeService.addApponeToCollectionIfMissing(this.apponesSharedCollection, post.applicationone);
  ...
  protected loadRelationshipsOptions(): void {
    this.apponeService
      .query()
      .pipe(map((res: HttpResponse<IAppone[]>) => res.body ?? []))
      .pipe(
        map((appones: IAppone[]) => this.apponeService.addApponeToCollectionIfMissing(appones, this.editForm.get('applicationone')!.value))
      )
      .subscribe((appones: IAppone[]) => (this.apponesSharedCollection = appones));
  ...
  protected createFromForm(): IPost {
    return {
      ...
      applicationone: this.editForm.get(['applicationone'])!.value,
