...
export class PostUpdatePage {
  ...
  applicationoneSelect = element(by.id('field_applicationone'));
  ...
  async applicationoneSelectLastOption(): Promise<void> {
    await this.applicationoneSelect.all(by.tagName('option')).last().click();
  }

  async applicationoneSelectOption(option: string): Promise<void> {
    await this.applicationoneSelect.sendKeys(option);
  }

  getApplicationoneSelect(): ElementFinder {
    return this.applicationoneSelect;
  }

  async getApplicationoneSelectedOption(): Promise<string> {
    return await this.applicationoneSelect.element(by.css('option:checked')).getText();
  }
